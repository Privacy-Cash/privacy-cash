...
import { prove, verify, Proof, parseProofToBytesArray, parseToBytesArray } from '../anchor/spl/tests/lib/prover';
import BN from 'bn.js';
import * as fs from 'fs';
import * as path from 'path';
import { utils } from 'ffjavascript';
import { MerkleTree } from '../anchor/spl/tests/lib/merkle_tree';
import { Utxo } from '../anchor/spl/tests/lib/utxo';
import { getExtDataHash, mockEncrypt } from '../anchor/spl/tests/lib/utils';
import { FIELD_SIZE, DEFAULT_HEIGHT } from '../anchor/spl/tests/lib/constants';
import { Keypair } from '../anchor/spl/tests/lib/keypair';
...
async function generateSampleProofForFirstDeposit(): Promise<{
  ...
  const input = {
    ...
    inputNullifier: inputNullifiers.map((n: any) => n.toString()),
    ...
    outputCommitment: outputCommitments.map((c: any) => c.toString()),
  ...
  try {
    ...
    console.log('- mintAddress:', input.mintAddress);
    console.log('- inputNullifier:', input.inputNullifier);
    ...
    console.log('- inBlinding[0]:', input.inBlinding[0]);
    console.log('- outPubkey[0]:', input.outPubkey[0]);
    ...
    publicSignals.forEach((signal: any, index: number) => {
    ...
    try {
      ...
      const res = await verify(path.resolve(__dirname, "../artifacts/spl/circuits/verifyingkey2.json"),
