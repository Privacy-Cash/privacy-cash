...
async function main() {
  try {
    ...
    console.log('\nBalances before transaction:', JSON.stringify({
      ...
      userWallet: `${userBalanceBefore / 1e9} SOL`,
    }, null, 2));
    ...
    const inputMerkleProofs = await Promise.all(
      inputs.map(async (utxo, index) => {
        ...
        if (utxo.amount.eq(new BN(0))) {
          return {
            pathElements: [...new Array(tree.levels).fill("0")],
            pathIndices: Array(tree.levels).fill(0)
          };
        }
    ...
    const instruction = new TransactionInstruction({
      keys: [
        ...
        // recipient - use the address from extData instead of user.publicKey
        { pubkey: extData.recipient, isSigner: false, isWritable: true },
        // fee recipient
    ...
    try {
      ...
      console.log('\nBalances after transaction:', JSON.stringify({
        ...
        userWallet: `${userBalanceAfter / 1e9} SOL`,
      }, null, 2));
      ...
      console.log('\nBalance changes:', JSON.stringify({
        ...
        userWallet: `${(userBalanceAfter - userBalanceBefore) / 1e9} SOL`,
      }, null, 2));
